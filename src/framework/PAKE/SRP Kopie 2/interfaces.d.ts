...
export interface BrowserOptions {
	...
	tokenPath?: string;
	callback?: (token: SharedVerifyResponse.token) => any;
}
...
export interface LoginState {
	...
	token: string;
	group?: SupportedRFC5054Bits;
	action: ACTION.R_SRP_AUTH | ACTION.R_SRP_EVIDENCE | ACTION.R_SRP_VERIFY /* evt. redirecting */;
}
export interface SharedLoginResponse { I: string; M2: string; action: any; token?: string; };
...
export interface SharedVerifyResponse { action: ACTION.R_ID_TOKEN; token: string; };
