import {
	BrowserOptions, BrowserRegister, BrowserChallenge, BrowserLogin,
...
export class Browser extends Base {
  ...
  constructor(_SRP: SupportedRFC5054Bits = 4096) {
	...
	static login(form: HTMLFormElement, options: BrowserOptions = {}) {
		const { challengePath, tokenPath, callback } = {
			challengePath: form.action,
			tokenPath: form.action,
			callback: null,
			...options
		}
		const { shared, secret } = Browser.formResult(form);
		...
		Browser.send(challengePath, cData, shared, function(res: SharedChallengeResponse) {
			...
			Browser.send(form.action, lData, shared, (res: SharedLoginResponse) => {
				...
				Browser.send(tokenPath, vData, shared, (res: SharedVerifyResponse) => {
					//{ action: ACTION.R_ID_TOKEN; token: string; }
					client.verify.call(client, res, callback);
				})
			})
		})
	}
  ...
  private crypto(res: SharedChallengeResponse): BrowserLogin {
	...
	private evidence(res: SharedLoginResponse) {
		...
		token = this.encrypt(token, this.getSessionSecret(this.s));
	...
	private verify(res: SharedVerifyResponse, callback: BrowserOptions['callback'] = null) {
		if (res.action !== Base.ACTION.R_ID_TOKEN) { throw new Error(Errors.IllegalStateException) }
		const token = this.decrypt(res.token, this.getSessionSecret(this.s));
