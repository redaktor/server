import {
	BrowserOptions, BrowserRegister, BrowserChallenge, BrowserLogin, BrowserToken,
...
export class Browser extends Base {
  ...
  constructor(_SRP: SupportedRFC5054Bits = 4096) {
	...
	static login(form: HTMLFormElement, status: any) {
			const challengePath = form.action;
		...
		Browser.send(challengePath, cData, shared, function(res: SharedChallengeResponse) {
			...
			Browser.send(form.action, lData, shared, (res: SharedLoginResponse) => {
				...
				Browser.send(form.action, vData, shared, (res: SharedVerifyResponse) => {
					const {U, ...idData} = client.verify.call(client, res, shared, callback);
					console.log('sending to',U,idData);
					Browser.send(U, idData, shared, function(res: any) {
						//console.log(res);
						window.location.href = res.href
					})
					if (typeof callback === 'function') { callback(idData.I) }
					//console.log('FINAL', res.id, u, u.searchParams.get('u'));
				})
			})
		})
	}
  ...
  private makeV(I: string = this.I, P: string = this.P) {
	...
	static send(path: string, data: BrowserChallenge|BrowserLogin|BrowserToken, pub: SharedSRP, cb: Function) {
		const { _csrf = '' } = pub;
