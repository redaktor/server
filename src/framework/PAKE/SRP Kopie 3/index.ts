...
export class Server extends Base {
  ...
  constructor(group: SupportedRFC5054Bits = 4096, hash: SupportedHashAlgo = 'SHA512') {
	...
	login(state: GenericState, serverLoginSecret: string): GenericResponse {
		if (state.action === Base.ACTION.R_SRP_AUTH) {
	...
	static authorize(me: any, token: string, serverLoginSecret: string) {
		try {
			...
			const idToken = jwt.decode(t, serverLoginSecret);
	...
	static isAuthorizedFn(serverLoginSecret: string) {
		const server = new Server();
		return (o: any) => {
			const {me, token} = o;
			const idToken = jwt.decode(server.decrypt(token, me.key), serverLoginSecret);
			try {
  ...
  private randomB() {
	...
	static parseJSON(json: string, additionalFields: FormFields = {}) {
	  ...
	  const o = JSON.parse(json);
		const { I, action } = o;
		if (!I || !(action in Base.ACTION)) { return false }
