...
class jwt {
  ...
  static verify(token: JWTFULL, key: string, method: string = 'HS256'): boolean {
    const { header, payload, signature } = token;
    const vText = [header, payload].join('.');
    const { alg } = JSON.parse(urlDecode(header));
    if (!alg || !algorithmMap.hasOwnProperty(alg)) { return false }
    const signMethod = (method === 'RSA-SHA256') ? method : null;
    if (signMethod) {
      if (!has('host-node')) {
      ...
      return crypto.verify(vText, key, method, unescape(signature));
    } else {
