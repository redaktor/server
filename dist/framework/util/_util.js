(function (deps, factory) {
    ...
    const TYPES = {
        ...
        glob: {
            ...
            is: function (v) { return ((v instanceof RegExp) && v.ignoreCase); },
            to: function (v) {
                var s = v.replace(/([\\|\||\(|\)|\[|\{|\^|\$|\*|\+|\?|\.|\<|\>])/g, function (x) { return '\\' + x; }).replace(/\\\*/g, '.*').replace(/\\\?/g, '.?');
                if (s.substring(0, 2) !== '.*') {
                    s = '^' + s;
                }
                else {
    ...
    function convert(value, typeOrSchema, evtFailValue) {
        this.res = void 0;
