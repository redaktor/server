...
const Noun = {
    ...
    isPluralFallback: (s) => (s.length > 3 && !/s$/.test(s) === true && /ss$/.test(s)),
    makeArticle: (t) => {
        const irregArticle = {
        ...
        let s = t.normal;
        if (irregArticle.hasOwnProperty(s)) {
            return irregArticle[s];
        }
        if (t.isAcronym && anAcronyms.hasOwnProperty(s.substr(0, 1))) {
            return 'an';
        }
        for (let i = 0; i < aRegexps.length; i++) {
            if (aRegexps[i].test(s)) {
                return 'a';
            }
        }
...
//# sourceMappingURL=irregularNoun.js.map