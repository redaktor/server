...
const Verb = {
    ...
    isPassive: (ts) => {
        if (ts.match('is being #PastTense').found ||
            ts.match('(had|has) been #PastTense').found) {
            return true;
        }
        return ts.match('will have been #PastTense').found;
    },
    isPerfect: (ts) => ts.match('^(had|have) #PastTense'),
    isImperative: (ts) => ts,
    isConditional: (ts) => ts,
    toBe: (isPlural = false, isNegative = false) => {
        let o = {
    ...
    correctConjugate: (o, isNegative = false) => {
        if (isNegative) {
...
for (let k in compact) {
    for (let i = 0; i < compact[k].length; i++) {
        Verb.suffixRules[compact[k][i]] = k;
    }
}
const hasY = /[bcdfghjklmnpqrstvwxz]y$/;
Verb.generic = {
    Gerund: (s) => (s.charAt(s.length - 1) === 'e' ? s.replace(/e$/, 'ing') : `${s}ing`),
    PresentTense: (s) => {
