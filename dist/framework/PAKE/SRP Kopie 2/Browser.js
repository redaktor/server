...
class Browser extends Base_1.Base {
    ...
    static login(form, options = {}) {
        const { challengePath, tokenPath, callback } = Object.assign({ challengePath: form.action, tokenPath: form.action, callback: null }, options);
        const { shared, secret } = Browser.formResult(form);
        const cData = {
            I: Browser.hash(shared.identity),
            action: Base_1.Base.ACTION.R_SRP_AUTH
        };
        Browser.send(challengePath, cData, shared, function (res) {
            ...
            Browser.send(form.action, lData, shared, (res) => {
                ...
                Browser.send(tokenPath, vData, shared, (res) => {
