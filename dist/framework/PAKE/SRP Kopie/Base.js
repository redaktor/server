...
class Base extends Base_1.PakeBase {
    ...
    encrypt(message, key = this.K) {
        ...
        const iv = util.createBuffer(this.randomBytes(16));
        console.log(key.length, 'IV', iv.length());
    ...
    decrypt(data, key = this.K) {
        ...
        let [iv, msg, tag] = [[0, 24], [24, end], [end]].map((a) => util.decode64(data.slice(...a)));
        console.log('decrypt', data.length, iv.length, msg.length, tag.length);
        dc.start({ iv, tag });
...
exports.Base = Base;
Base.ACTIONS = constants_1.ACTIONS;
