        ...
        additionalProperties: [
            {
                ...
                when: function (o) { return (o.providerID === 'mail'); }
            },
            {
                type: 'input',
                name: 'provider_name',
                message: function () {
                    return log_1._ `${this.msg('qName')} muted${' < 64 '} ${this.msg('chars')}`;
                },
                default: 'IndieAuth',
                validate: function (value) {
            ...
            },
            {
                ...
                name: 'provider_host',
                message: function () {
                    return log_1._ `${this.msg('qHost')}\nmuted${this.msg('qHostNote')}`;
                },
                default: function (o) { return (o.provider_email.split('@')[1]); },
                when: function (o) { return (o.providerID === 'mail'); }
            },
            {
                type: 'list',
                ...
                choices: function (o) {
                    ...
                    return new Promise((resolve, reject) => {
                        if (!this || !this.scanPort) {
                            return;
                        }
                        Promise.all(ports.map((_p) => this.scanPort(_p, host))).then((pr) => {
                            resolve(pr.map((status, i) => {
                                const __p = ports[i].toString();
                                const msg = __p + ' (seems to be ' + status + ')';
                                return {
                                    name: status === 'open' ? log_1._ `G{msg}` : log_1._ `R{msg}`,
                                    value: __p,
                                    short: 'port ' + __p
                                };
                            }));
