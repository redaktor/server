...
class IndieAuthClient extends Request_1.default {
    constructor(_url = '', _params = {}, verifyTimeout = config_1.verifyTimeout + 400, el = {}, colors, sentProgress = {}, messages = {}) {
        super();
        this._url = _url;
        ...
        this.el = el;
        this.colors = colors;
        this.sentProgress = sentProgress;
        ...
        this._each = Array.prototype.slice;
        this.isObject(_url) && util_1.mixin(this, _url);
        this._url = this._normalizeUrl(window.location.href, false);
        this._params = url_1.default.parameters(window.location.href);
    ...
    reorderProgress() {
        ...
        this._each.call(document.querySelectorAll(ps)).map((el, i) => {
    ...
    setProgress(percent, sel = '.ui.progress', barColor = '') {
        if (!this.hasProgress()) {
            return;
        }
    ...
    iconMessage(msg, iconCl = '', colorCl = '', isMeta = true) {
        if (iconCl !== '') {
            iconCl = iconCl + ' icon';
        }
    ...
    verifyUI(el, error = null, readyCount = 0) {
        if (error) {
        ...
        const meUrl = document.querySelector('.me.site.title')['dataset']['ref'];
        let order = 4;
        let _end = false;
        if (this.hasProgress()) {
        ...
        if (this.el.grid.querySelectorAll(this.s.done).length === readyCount) {
            ...
            this._each.call(this.el.rescan).map((r) => {
    ...
    verify(cacheBust = false) {
        ...
        let hasCache = false;
        this._each.call(this.el.providers).map((el) => {
    ...
    authUI(el) {
        ...
        [].forEach.call(document.querySelectorAll('.gpgAuthForm'), (fEl) => {
            ...
            if (!!this.hasProgress() && !!dimmer) {
                ...
                this.sentProgress[el.dataset.provider] = setInterval(() => {
                    ...
                    if (per > 99 && pEl.hasClass('yellow')) {
                        ...
                        if (cEl.hasClass('icon')) {
                            cEl.removeClass('icon');
                        }
